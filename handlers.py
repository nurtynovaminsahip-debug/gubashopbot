...
async def cb_pay_stars_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u:
        await query.edit_message_text("❌ Товар не найден.")
        return
    stars_qty = u.get("stars_price") or math.ceil(u["price_rub"] / 1.29)
    await query.edit_message_text(
        f"⭐️ Счёт на оплату отправлен ниже ⬇️\n\n"
...
async def cb_dynpay_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not p:
        await query.edit_message_text("❌ Товар не найден.")
        return
    stars_qty = p.get('stars_price') or math.ceil(p['price_rub'] / 1.29)
    await query.edit_message_text(
        f"⭐️ Счёт на оплату отправлен ниже ⬇️\n\n"
        f"📦 Товар: {p['label']}\n"
        f"💳 Сумма: {stars_qty} ⭐️"
    ...
    await context.bot.send_invoice(
        chat_id=query.from_user.id,
        title=p['label'],
        description=p.get('description') or p['label'],
        payload=f"dynprod_{prod_id}",
        currency="XTR",
        prices=[LabeledPrice(p['label'], stars_qty)],
    )
