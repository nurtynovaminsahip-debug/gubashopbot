...
async def cb_admin_top_manage(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not await db.is_admin(query.from_user.id):
        return
    entries = await db.get_custom_top_buyers()
    text = "🏆 <b>Управление топом покупателей</b>\n\n"
    if entries:
        text += "<i>Ручные записи (нажмите ❌ чтобы удалить):</i>\n\n"
        for e in entries:
            text += f"• <b>{e['display_name']}</b> — {e['total_spent']:.2f}₽, {e['order_count']} покупок\n"
    else:
        text += "<i>Ручных записей пока нет. Реальные покупатели подтягиваются автоматически.</i>"
    await query.edit_message_text(text, reply_markup=kb.admin_top_manage_keyboard(entries), parse_mode="HTML")


async def cb_admin_top_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not await db.is_admin(query.from_user.id):
        return
    entry_id = int(query.data.split("_")[-1])
    await db.remove_custom_top_buyer(entry_id)
    entries = await db.get_custom_top_buyers()
    text = "🏆 <b>Управление топом покупателей</b>\n\n"
    if entries:
        text += "<i>Ручные записи:</i>\n\n"
        for e in entries:
            text += f"• <b>{e['display_name']}</b> — {e['total_spent']:.2f}₽, {e['order_count']} покупок\n"
    else:
        text += "✅ <i>Запись удалена. Ручных записей нет.</i>"
    await query.edit_message_text(text, reply_markup=kb.admin_top_manage_keyboard(entries), parse_mode="HTML")


async def cb_admin_top_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not await db.is_admin(query.from_user.id):
        return
    context.user_data['admin_top_new'] = {}
    await query.edit_message_text(
        "🏆 <b>Добавить в топ — шаг 1/3</b>\n\n"
        "Введите <b>отображаемое имя</b> покупателя\n"
        "<i>(например: @username или «Аноним»)</i>",
        reply_markup=kb.admin_top_cancel_keyboard(),
        parse_mode="HTML"
    )
    return states.ADMIN_TOP_ADD_NAME


async def handle_admin_top_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    name = update.message.text.strip()
    context.user_data['admin_top_new']['name'] = name
    await update.message.reply_text(
        f"✅ Имя: <b>{name}</b>\n\n"
        "🏆 <b>Шаг 2/3</b>\n\n"
        "Введите сумму потраченных рублей (например: <code>15000</code>):",
        reply_markup=kb.admin_top_cancel_keyboard(),
        parse_mode="HTML"
    )
    return states.ADMIN_TOP_ADD_SPENT


async def handle_admin_top_spent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        spent = float(update.message.text.strip().replace(",", "."))
    except ValueError:
        await update.message.reply_text("❌ Введите число, например <code>15000</code>:", parse_mode="HTML")
        return states.ADMIN_TOP_ADD_SPENT
    context.user_data['admin_top_new']['spent'] = spent
    name = context.user_data['admin_top_new']['name']
    await update.message.reply_text(
        f"✅ Сумма: <b>{spent:.2f}₽</b>\n\n"
        "🏆 <b>Шаг 3/3</b>\n\n"
        "Введите <b>количество покупок</b> (например: <code>7</code>):",
        reply_markup=kb.admin_top_cancel_keyboard(),
        parse_mode="HTML"
    )
    return states.ADMIN_TOP_ADD_ORDERS


async def handle_admin_top_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        orders = int(update.message.text.strip())
    except ValueError:
        await update.message.reply_text("❌ Введите целое число, например <code>7</code>:", parse_mode="HTML")
        return states.ADMIN_TOP_ADD_ORDERS
    data = context.user_data.get('admin_top_new', {})
    name = data.get('name', '—')
    spent = data.get('spent', 0.0)
    await db.add_custom_top_buyer(name, spent, orders)
    context.user_data.pop('admin_top_new', None)
    entries = await db.get_custom_top_buyers()
    text = (
        f"✅ <b>Запись добавлена в топ!</b>\n\n"
        f"👤 {name} — {spent:.2f}₽, {orders} покупок\n\n"
        "🏆 <b>Текущие ручные записи:</b>\n"
    )
    for e in entries:
        text += f"• <b>{e['display_name']}</b> — {e['total_spent']:.2f}₽, {e['order_count']} покупок\n"
    await update.message.reply_text(text, reply_markup=kb.admin_top_manage_keyboard(entries), parse_mode="HTML")
    return ConversationHandler.END
