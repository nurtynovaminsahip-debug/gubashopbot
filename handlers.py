...
async def cb_pay_stars_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    p = kb.PREMIUM_PRICES[plan]
    await query.edit_message_text(
        f"⭐️ Счёт на оплату отправлен ниже ⬇️\n\n"
        f"📦 Товар: Telegram Premium {p['label']}\n"
        f"💳 Сумма: {p['stars']} ⭐️"
    )
...
async def cb_pay_stars_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not stars_qty:
        await query.edit_message_text("❌ Цена в звёздах для этого товара не задана.")
        return
    await query.edit_message_text(
        f"⭐️ Счёт на оплату отправлен ниже ⬇️\n\n"
        f"🧩 Товар: Username @{key}\n"
        f"💳 Сумма: {stars_qty} ⭐️"
    )
    await context.bot.send_invoice(
        chat_id=query.from_user.id,
        title=f"Username @{key}",
        description=f"Покупка Telegram Username @{key} через GubaShop",
...
async def cb_dynprod(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'stars' in methods:
        ...
        stars_line = f"\n 💳 Стоимость в звёздах: {stars_qty} ⭐️" if stars_qty else ""
    text = (
        f"🛒 {p['label']}\n\n"
    ...
    await query.edit_message_text(
        text,
        reply_markup=kb.dynamic_product_keyboard(prod_id, methods, price_rub, stars_price=p.get('stars_price'))
    )
...
async def cb_dynpay_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_invoice(
            ...
            provider_token="",
            currency="XTR",
            prices=[LabeledPrice(label, stars_qty)],
        )
        await query.edit_message_text(
            f"⭐️ Счёт на оплату отправлен ниже ⬇️\n\n"
            f"📦 Товар: {p['label']}\n"
...
def _wizard_text(draft: dict) -> str:
    ...
    if 'stars' in selected and draft.get('stars_price'):
        stars_line = f"\n ⭐️Цена в звёздах: {draft['stars_price']} ⭐️"
    return (
...
async def cb_adminwiz_togglepay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if method in selected:
        ...
        draft['payment_methods'] = selected
        if method == 'stars':
            current = draft.get('stars_price', '')
            hint = f" (текущая: {current} ⭐️)" if current else ''
            await query.edit_message_text(
                f"⭐️ Введите цену товара в звёздах (целое число){hint}:\n\n"
                f"Например: 100",
...
async def handle_wizard_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        stars = int(text)
        if stars <= 0:
            await update.message.reply_text("❌ Введите положительное целое число:", reply_markup=kb.wizard_text_back_keyboard())
            return states.WIZARD_STARS
        ...
        try:
            ...
            await update.message.reply_text(_wizard_text(draft), reply_markup=kb.wizard_payment_keyboard(selected))
        return states.WIZARD_MAIN
    except ValueError:
        await update.message.reply_text("❌ Введите целое число:", reply_markup=kb.wizard_text_back_keyboard())
        return states.WIZARD_STARS
...
async def handle_successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("premium_"):
        ...
        await update.message.reply_text("✅ Оплата принята! Ожидайте выполнения заказа.")
        ...
        if p:
            ...
            if p.get('delivery_type') == 'auto':
                delivered = await _auto_deliver(context, user_id, p['id'], p['label'])
                if delivered:
                    await update.message.reply_text("✅ Оплата принята! Ваш товар отправлен выше ⬆️")
                    return
            await _notify_payment(context, user_id, p['label'], p['price_rub'], f"Stars ({stars} ⭐️)", order_id)
        await update.message.reply_text("✅ Оплата принята! Ожидайте выполнения заказа.")

    else:
        await update.message.reply_text("✅ Оплата получена! Ожидайте выполнения заказа.")
