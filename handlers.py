...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_data or not user_data["agreed_to_policy"]:
        await update.message.reply_text(
            ...
            "Перед использованием бота ознакомьтесь с политикой нашего проекта и примите условия:",
...
async def cb_stars_continue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not qty or qty < 50:
        ...
        return
    rate = await _get_stars_rate()
    recipient = await db.get_setting("stars_recipient", "")
    recipient_note = f"\n\n👤 <i>Получатель звёзд:</i> <b>{recipient}</b>" if recipient else ""
    await query.edit_message_text(
        f"💳 <b>Выберите способ оплаты</b>\n\n"
        f"⭐️ <b>Количество:</b> {qty} звёзд\n"
        f"💰 <b>Сумма:</b> {round(qty * rate, 2):.2f}₽{recipient_note}",
...
async def cb_pay_crypto_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"⭐️ <i>Количество:</i> <b>{qty} звёзд</b>\n"
...
async def cb_robux_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "⏱ <i>Срок:</i> <b>14 рабочих дней</b>\n\n"
        "💰 <i>Курс:</i> <b>1 R$ = 60₽ | 1 R$ = 45 ⭐️</b>\n\n"
        "📋 <i>Требования:</i>\n"
        "• Зайдите в указанную группу Roblox\n"
        "• Робуксы придут через 14 дней после зачисления\n\n"
        "Укажите количество и никнейм группы:"
    )
    keyboard = kb.robux_group_keyboard(grp['qty'], grp['recipient'])
    if banner:
        try:
            await query.message.reply_photo(photo=banner, caption=text, parse_mode="HTML", reply_markup=keyboard)
            await query.message.delete()
...
async def handle_robux_group_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        grp['qty'] = qty
        await update.message.reply_text(
            f"✅ <b>Количество: {qty} R$</b>\n💰 Стоимость: <b>{qty * 60:.0f}₽</b>",
            reply_markup=kb.robux_group_keyboard(qty, grp.get('recipient', '')),
            parse_mode="HTML"
        )
        return ConversationHandler.END
    except ValueError:
...
async def cb_robux_group_recipient(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_text(
        "👤 <b>Введите никнейм вашей группы в Roblox:</b>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="robux_group")]]),
        parse_mode="HTML"
    )
...
async def handle_robux_group_recipient(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ <b>Никнейм группы: {recipient}</b>",
        reply_markup=kb.robux_group_keyboard(grp['qty'], recipient),
        parse_mode="HTML"
    )
    return ConversationHandler.END
...
async def cb_robux_group_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not qty:
        ...
        return
    if not recipient:
        await query.answer("❌ Укажите никнейм группы!", show_alert=True)
        return
    await query.edit_message_text(
        f"💳 <b>Оплата Robux (Группой)</b>\n\n"
        f"💎 <b>{qty} R$</b> → группа <b>{recipient}</b>\n"
        f"💰 <b>{qty * 60:.0f}₽</b>\n\nВыберите способ оплаты:",
        reply_markup=kb.robux_group_payment_keyboard(qty, recipient),
        parse_mode="HTML"
    )
...
async def cb_robux_group_pay_crypto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price = round(qty * 60 * 1.015, 2)
    usdt = await get_crypto_bot_commission_amount(qty * 60)
...
async def cb_robux_group_pay_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_invoice(
            ...
            description=f"{qty} Robux через группу → {recipient}",
...
async def cb_roblox_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if search_ctx == 'gamepass':
        ...
        await query.edit_message_text(f"✅ <b>Никнейм группы: {roblox_name}</b>", parse_mode="HTML")
        ...
        await query.edit_message_text(f"✅ <b>Получатель выбран: {roblox_name}</b>", parse_mode="HTML")
...
async def cb_roblox_search_again(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data=f"robux_{search_ctx}" if search_ctx in ['gamepass', 'group'] else "robux_superpasses")]]),
        parse_mode="HTML"
    )
    return states.ROBUX_USERNAME_SEARCH
...
async def _notify_payment(context, user_id: int, product: str, amount: float, method: str, order_id: int):
    ...
    stars_recipient = await db.get_setting("stars_recipient", "")
    recipient_note = f"\n👤 <i>Получатель звёзд:</i> <b>{stars_recipient}</b>" if stars_recipient and "Stars" in method else ""
    ...
    await notify_all_admins(
        ...
        f"💰 <i>Сумма:</i> <b>{amount:.2f}₽</b>\n"
        f"💳 <i>Способ:</i> <b>{method}</b>{recipient_note}",
        reply_markup=InlineKeyboardMarkup([
