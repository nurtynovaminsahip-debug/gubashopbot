...
def admin_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏆 Управление топом покупателей", callback_data="admin_top_manage")],
    ])


def admin_top_manage_keyboard(custom_entries: list) -> InlineKeyboardMarkup:
    rows = []
    if custom_entries:
        for e in custom_entries:
            rows.append([
                InlineKeyboardButton(
                    f"❌ {e['display_name']} — {e['total_spent']:.2f}₽",
                    callback_data=f"admin_top_remove_{e['id']}"
                )
            ])
    rows.append([InlineKeyboardButton("➕ Добавить в топ", callback_data="admin_top_add")])
    rows.append([InlineKeyboardButton("🔙 Назад", callback_data="admin_back")])
    return InlineKeyboardMarkup(rows)


def admin_top_cancel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ Отмена", callback_data="admin_top_manage")]
    ])
