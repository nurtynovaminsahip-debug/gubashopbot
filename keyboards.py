...
def policy_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✅ Я ознакомился и принимаю условия", callback_data="agreed_policy")]
    ])
...
def stars_quantity_keyboard(selected: int = 0):
    return InlineKeyboardMarkup([
        [
            ...
            InlineKeyboardButton("⭐️ 100 звёзд", callback_data="stars_100"),
        ],
        [
            InlineKeyboardButton("⭐️ 150 звёзд", callback_data="stars_150"),
            InlineKeyboardButton("⭐️ 200 звёзд", callback_data="stars_200"),
        ],
        [InlineKeyboardButton("✏️ Ввести своё количество", callback_data="stars_custom")],
        [InlineKeyboardButton("▶️ Продолжить к оплате", callback_data="stars_continue")],
...
def stars_recipient_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✏️ Другому человеку", callback_data="stars_recipient_other")],
...
def confirm_balance_keyboard(callback_yes: str, callback_no: str = "cancel_payment"):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Подтвердить оплату", callback_data=callback_yes)],
...
def dynamic_product_info_keyboard(product_id: int, delivery_type: str, qty: int, price_rub: float,
    ...
    buttons = [
        [
            InlineKeyboardButton(f"⚡️ Тип выдачи:\n{delivery_label}", callback_data="noop_info"),
            InlineKeyboardButton(f"📦 В наличии:\n{qty} шт.", callback_data="noop_info"),
        ],
        [
            InlineKeyboardButton(f"🏷 Стоимость:\n{price_rub:.0f}₽", callback_data="noop_info"),
            InlineKeyboardButton("💳 Купить", callback_data=f"dynbuy_{product_id}"),
        ],
        [InlineKeyboardButton("🔙 Назад", callback_data="back_to_subcategories")]
    ]
    return InlineKeyboardMarkup(buttons)
...
def robux_group_keyboard(qty: int = 0, recipient: str = ""):
    qty_text = f"{qty} R$" if qty else "не выбрано"
    rec_text = recipient if recipient else "не выбран"
    price_rub = round(qty * 60, 2) if qty else 0
    price_stars = round(qty * 45) if qty else 0
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"✏️ Количество: {qty_text}", callback_data="robux_group_qty")],
        [InlineKeyboardButton(f"👤 Никнейм группы: {rec_text}", callback_data="robux_group_recipient")],
        [InlineKeyboardButton(f"💳 Купить" + (f" — {price_rub:.0f}₽" if qty else ""), callback_data="robux_group_buy")],
        [InlineKeyboardButton("🔙 Назад", callback_data="robux_main")],
    ])
...
def robux_group_payment_keyboard(qty: int, recipient: str):
    price_rub = round(qty * 60, 2)
    price_stars = round(qty * 45)
    crypto_price = round(price_rub * 1.015, 2)
    yoo_price = round(price_rub * 1.03, 2)
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🤖 Crypto Bot — {crypto_price:.0f}₽", callback_data=f"robux_group_pay_crypto_{qty}")],
        [InlineKeyboardButton(f"💳 YooMoney — {yoo_price:.0f}₽", callback_data=f"robux_group_pay_yoo_{qty}")],
        [InlineKeyboardButton(f"💰 Баланс — {price_rub:.0f}₽", callback_data=f"robux_group_pay_balance_{qty}")],
