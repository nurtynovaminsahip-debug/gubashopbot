...
def policy_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✅ Принимаю оба документа", callback_data="agreed_policy", api_kwargs={"style": "success"})]
    ])
...
def subscribe_check_keyboard(channels: list):
    ...
    buttons.append([InlineKeyboardButton("✅ Я подписался — проверить", callback_data="check_subscriptions", api_kwargs={"style": "success"})])
    return InlineKeyboardMarkup(buttons)
...
def stars_quantity_keyboard(selected: int = 0):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✅ Продолжить к оплате", callback_data="stars_continue", api_kwargs={"style": "primary"})],
...
def payment_keyboard_stars(qty: int, rate: float = STARS_RATE):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"💰 Баланс — {price:.2f}₽", callback_data=f"pay_balance_stars_{qty}", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("🔙 Назад", callback_data="sub_stars")]
    ])
...
def confirm_balance_keyboard(callback_yes: str, callback_no: str = "cancel_payment"):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Оплатить с баланса", callback_data=callback_yes, api_kwargs={"style": "success"})],
        [InlineKeyboardButton("❌ Отмена", callback_data=callback_no, api_kwargs={"style": "danger"})]
    ])
...
def profile_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💰 Пополнить баланс", callback_data="deposit", api_kwargs={"style": "primary"})],
...
def wizard_main_keyboard(has_stars: bool = False, has_banner: bool = False):
    ...
    rows.append([InlineKeyboardButton("✅ Добавить товар", callback_data="adminwiz_confirm", api_kwargs={"style": "success"})])
    rows.append([InlineKeyboardButton("❌ Отмена", callback_data="adminwiz_cancel", api_kwargs={"style": "danger"})])
...
def order_chat_user_keyboard(order_id: int):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✅ Завершить чат и подтвердить", callback_data=f"chat_close_{order_id}", api_kwargs={"style": "success"})],
    ])
...
def order_chat_admin_keyboard(order_id: int, user_id: int):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✅ Подтвердить заказ", callback_data=f"admin_order_done_{order_id}_{user_id}", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("❌ Отклонить заказ", callback_data=f"admin_order_reject_{order_id}_{user_id}", api_kwargs={"style": "danger"})],
    ])
...
def contest_detail_keyboard(contest_id: int, is_participant: bool):
    ...
    if not is_participant:
        buttons.append([InlineKeyboardButton("✅ Участвовать в конкурсе", callback_data=f"contest_join_{contest_id}", api_kwargs={"style": "success"})])
    else:
...
def robux_group_keyboard(qty: int = 0, recipient: str = "", rate_rub: float = 0.60, rate_stars: float = 0.45):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"✅ Купить" + (f" — {price_rub:.2f}₽" if qty else ""), callback_data="robux_group_buy", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("🔙 Назад", callback_data="robux_main")],
    ])
